...
// Verifies that a leaf certificate chains to a trusted CA and is valid at the given timestamp.
// Adapted from: https://github.com/sigstore/sigstore-js/blob/main/packages/verify/src/key/certificate.ts
//
// This function filters certificate authorities by validity period and tries to verify
// the certificate chain against each CA until one succeeds or all fail.
//
// Changes from sigstore-js:
// - Async (uses await) because CertificateChainVerifier.verify() is async for Web Crypto API
// - Does not use VerificationError class (consumers can wrap errors as needed)
// - Filters CAs by timestamp inline rather than using separate filterCertAuthorities function
export async function verifyCertificateChain(
  timestamp: Date,
  leaf: import("./x509/cert.js").X509Certificate,
  certificateAuthorities: import("./interfaces.js").CertAuthority[]
): Promise<import("./x509/cert.js").X509Certificate[]> {
  let lastError: Error | undefined;

  for (const ca of certificateAuthorities) {
    // Filter CAs by timestamp validity
    if (timestamp < ca.validFor.start || timestamp > ca.validFor.end) {
      continue;
    }

    try {
      const { CertificateChainVerifier } = await import("./x509/chain.js");
  ...
  // If we failed to verify the certificate chain for all trusted CAs,
  // throw the last error we encountered
  throw new Error(
    `Failed to verify certificate chain: ${lastError?.message || 'No valid CAs found for timestamp'}`
  );
}
