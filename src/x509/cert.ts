...
Adapted from: https://github.com/sigstore/sigstore-js/blob/main/packages/core/src/x509/cert.ts
Changes: Buffer → Uint8Array, Node.js crypto → Web Crypto API, uses Fulcio extensions
*/
...
export class X509Certificate {
  ...
  public async getPublicKeyObj(hashAlg?: string): Promise<CryptoKey> {
    ...
    if (algorithmOID === "1.2.840.113549.1.1.1") {
      ...
      const hash = hashAlg || HashAlgorithms.SHA256;
      let hashAlgorithm: HashAlgorithms;
      ...
      // Convert "SHA-256" to "SHA256" format for importKey
      const scheme = hashAlgorithm.replace("-", "");
